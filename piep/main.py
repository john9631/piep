...
from piep._builtins import builtins
from piep.error import Exit

if sys.version_info[0] > 2:
	imap = map
	basestring = str
	from piep.exec3 import exec3 as _exec
else:
	imap = itertools.imap
	from piep.exec2 import exec2 as _exec
		...
		for line in output:
			if line is not None:
				if isinstance(line, (tuple, list)):
					line = opts.join.join(list(map(str, line)))
				yield line
